...
- **Small diff** (typo / copy, < 50 changed lines): explicit exception —
  ...
  main-session** = `Claude Agent + codex`; **per-slice (main=Claude)** =
  `codex + agy` (still cross-family — a slice subagent cannot spawn the
  Claude `Agent`, and main=Claude per-slice carries no Claude leg). The
  2-vendor pick MUST still satisfy the §Step-1 host minimum-leg guarantee
  (so main=Codex 的小 diff 双腿 = `codex + claude -p`). You MUST annotate
  the eventual
...
its old rationale — "Claude credit is too tight for per-slice, concentrate
Claude to ship-pre" — is **superseded 2026-07-14** by the minimum-leg
guarantee below; the shapes survive where they already satisfy it):

> **⚠ RECORDED RULE (2026-07-14) — 宿主最低腿保证 (host minimum-leg
> guarantee).** Keyed on the host (`main=X`, 宿主), EVERY review squad —
> per-slice AND ship-pre, both lenses — must include at least one strong
> leg from the OTHER family:
>
> - **main=Claude** → ≥1 codex `gpt-5.6-sol` leg.
> - **main=Codex** → ≥1 Claude `opus` (Opus 4.8) leg — in per-slice /
>   nested contexts satisfied by the headless `claude -p` CLI form (see
>   the main=Codex 宿主替换表).
> - **main=其他宿主** → at least one of codex / Claude.
>
> The old flag `不用 Claude (credit)` is repealed with the doctrine. This
> is a by-design composition floor, not outage magic: a true outage still
> degrades per §Step 3 (never silent).

- **per-slice** (after each slice's baseline commit — within-slice lens):
  **`N codex + agy` = 2-vendor by design** (main=Claude: the codex leg
  meets the minimum-leg guarantee; no Claude leg required here). Run by
  the slice's own
  ...
  (N+1)/(N+1) + flag `per-slice 双腿 by design`.
...
cmr does not use Fable (Step 2 recorded rule; main=Claude runs the
Claude leg via `Agent` at ship-pre; main=Codex 的 Claude 腿**一律**
`claude -p`, 宿主替换表); codex
...
2026-07-14; local `grok` CLI — form and family accounting in §Step 3).
Only codex instantiates by diff
...
| Claude down | Codex + Gemini；flag `本轮缺 claude`（最低腿保证被 outage 打破——仅真 outage 可走此行，never silent）。 |
| Gemini down | Codex + Claude + **grok 顶替**（§Step 3 顶替形式）；flag `agy down → grok-4.5 high 顶替`。grok 亦不可用 → Codex + Claude，flag `本轮缺 gemini（grok 亦不可用）`。 |
| Claude + Gemini both down | Codex + **grok**（grok 独担 outside voice）；flag `本轮缺 claude；agy down → grok 顶替`。grok 亦不可用 → Codex only，flag `本轮无 outside voice — 需人工补 review`（不可正向终止，同 §Step 5 单 vendor 行）。 |
| ~~固定双腿场景~~（**repealed 2026-07-14**，宿主最低腿保证） | main=Codex 的 per-slice / correctness (Step 6) 不再是 `codex + agy` 双腿：**必须带 Claude `opus` 腿**（上方 Claude reviewer 腿行的 `claude -p` CLI 形式）→ squad = `N codex + claude(-p) + agy` = N+1+1（N 按 Step 1 N 表，常态 N=1 即 1+1+1）；completeness (Step 5) 照旧 N+1+1。旧 flag `不用 Claude (credit)` 随废。计分同 mainline 对应行（(N+2)/(N+2) concur 等）。 |
| ~~codex-solo 正向终止例外~~（**repealed 2026-07-14**） | Claude 腿转为强制 + agy down 由 grok 顶替后，agy down 不再产生 codex-solo：squad 仍 ≥ `codex + claude(-p)`（+grok）。codex solo 只可能在 claude、agy、grok 全灭时出现——按上方 both-down 行处理，**不可正向终止**。 |
...
| codex + gemini both | Claude + **grok**（顶替 agy 位，outside voice 保住） | `本轮缺 codex；agy down → grok 顶替`。grok 亦不可用 → Claude only，`本轮无 outside voice — 需人工补 review` |

> **grok 顶替形式（RECORDED 2026-07-14）：** agy 不可用时第三腿由本地
> `grok` CLI（Grok Build）顶替：`grok --prompt-file "$PROMPT_FILE" -m
> grok-4.5 --reasoning-effort high`（`--effort` 同义；headless 单轮，
> stdout 即 review 文本，喂同一份 lens prompt + diff）。**家族计法：**
> grok 属 xAI、非 Google family——合并判定不得计入 Google-family
> diversity，round report 必须注明顶替；但它计一条**成功腿**（同 agy
> step-down 的计法先例）。grok 自身 outage（空 stdout / 非零 exit）→
> 回退本表原缩编行为并 flag，never silent。
...
> legs approve（`complete` / `converged`，按所派 lens 的 verdict 词表）
> AND the ledger — aggregating ALL legs' findings,
> including any leg that dissented from the majority-approve vote — shows
...
> ledger non-zero → the round is **NOT clear**, even under a majority-approve
...
- **By-design 2-vendor (main=Claude per-slice)**: fixed `N codex + agy`
  ...
  **(N+1)/(N+1) concur + flag `per-slice 双腿 by design`**, scored the
  ...
  completeness 与 Step 6 correctness — 无论宿主一律含 Claude 腿的
  N+1+1（N 按 Step 1 N 表，常态 N=1 即 1+1+1），不适用本行；
...
- Only 1 vendor ran (no outside voice — e.g. codex + gemini + grok ALL down → Claude only): **NOT positive** — no cross-family check; needs human review or wait for vendor recovery.
