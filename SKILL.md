...
| ~~固定双腿场景~~（**repealed 2026-07-14**，宿主最低腿保证） | main=Codex 的 per-slice / correctness (Step 6) 不再是 `codex + agy` 双腿：**必须带 Claude `opus` 腿**（上方 Claude reviewer 腿行的 `claude -p` CLI 形式）→ squad = `codex + claude(-p) + agy` = 1+1+1；completeness (Step 5) 照旧 1+1+1。旧 flag `不用 Claude (credit)` 随废。计分同 mainline 对应行（3/3 concur 等）。 |
