...
> 2026-09-05（ADR 0005）：§规则→唯一权威位置映射 中指向 `DOC-MODE.md`
> （ADR 0004 §8 已删）、`TESTING.md`、`SKILL.md` Step 2「Reasoning-effort
> contract」（均随 v0.5 删除）的行自此为历史记录；现役权威位置只有
> `SKILL.md` 各 Step、两份 lens prompt 与 `CONTEXT.md`。
...
|---|---|---|
| main=Claude per-slice 无 Claude 腿（2026-07-14 宿主最低腿保证后；main=Codex per-slice 带 `claude -p`） | `SKILL.md` Step 1 `setup: who runs it decides the squad + the N table` / §谁跑 | description 层维持缺席（不得回流机制词；留痕由 RECORDED 标记 + git 史，#38 后无测试守护），非正向指针；main=Codex 宿主替换表 |
