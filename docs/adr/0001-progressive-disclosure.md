...
|---|---|---|
| per-slice 无 Claude | `SKILL.md` Step 1 `setup: who runs it decides the squad + the N table` / §谁跑 | description 层维持缺席（不得回流机制词；留痕由 RECORDED 标记 + git 史，#38 后无测试守护），非正向指针；main=Codex 宿主替换表 |
