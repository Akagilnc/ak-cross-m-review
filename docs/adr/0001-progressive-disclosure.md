...
拆分后只有两个 freeze boundary：`SKILL.md` residual text hash 与
`DOC-MODE.md` whole-file hash。迁移必须在同一个 commit 中原子完成：先让测试同时
...
|---|---|---|
| per-slice 无 Claude | `SKILL.md` Step 1 `setup: who runs it decides the squad + the N table` / §谁跑 | description 的 absence guard；main=Codex 宿主替换表 |
| Fable 禁用 | `SKILL.md` Step 2 Claude bullet 的 `RECORDED RULE（存续）— Fable 禁用` | main=Codex 宿主替换表对应行 |
| «严禁合一次 cmr 闸» | `SKILL.md` Step 0 completeness / correctness 两条 | 无 |
| codex effort default + override | `SKILL.md` Step 2 `Reasoning-effort contract` | `TESTING.md`、`README.md` 仅作操作 pointer |
| 缺钉闸 | `prompts/cmr-completeness.md` §缺钉闸 | 无 |
...
| step-down ≠ 降级 | `SKILL.md` Step 2 Gemini bullet | 无 |
...
- 两条 hash 与反双源断言形成无真空期保护；新增 ADR、CONTEXT 和 CLAUDE 映射
