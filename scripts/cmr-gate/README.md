...
### Vela-like 既有 hook 共存(non-`.githooks` hooksPath)

如果本仓 `core.hooksPath` 不是 `.githooks/`(e.g. vela 是 `.git/hooks/` 绝对路径,
且 `.git/hooks/post-commit` 已被本地 review-gate 占用),`.githooks/post-commit`
不会自动生效。chain cmr-gate L1 到既有 post-commit 末尾:
...
确认只 chain 一次。这是 vela 本次端用的法子(既有 review-gate-postcommit.sh 不动,
后面 append cmr-gate audit)。
